#!/usr/bin/env python3
"""
Fixed Metadata-First RAG Processor with correct path parsing
"""

# [Include all the same imports and class structure as before, but replace the discover_all_metadata_files function:]
...
class FixedMetadataFirstProcessor:
    def __init__(self):
        self.config = {
    ...
    def discover_all_metadata_files(self):
        """Discover all metadata JSON files across all seasons - FIXED VERSION"""
        all_metadata = []
        ...
        for blob in self.gcs_bucket.list_blobs(prefix=metadata_prefix):
            ...
            # FIXED: Look at path_parts[3] instead of path_parts[4]
            if len(path_parts) >= 5 and len(path_parts) > 3 and path_parts[3].startswith('season_'):
                folders.add(path_parts[3])
        ...
        for season_folder in season_folders:
            ...
            json_blobs = [blob for blob in self.gcs_bucket.list_blobs(prefix=season_prefix) 
            ...
            # Count processed vs unprocessed for this season
            processed_count = sum(1 for m in season_metadata if m["already_processed"])
            unprocessed_count = len(season_metadata) - processed_count
            
            self.log_progress(f"📁 {season_folder}: {len(season_metadata)} episodes ({processed_count} processed, {unprocessed_count} pending)")
    ...
    def extract_episode_metadata_from_json(self, metadata_json):
        ...
        return {
            ...
            "source_file": metadata_json.get("title", "")
        }
    ...
    def process_all_episodes(self):
        ...
        self.log_progress("🚀 Starting Fixed Metadata-First RAG Processing")
        ...
        self.log_progress(f"\n🏁 Fixed Metadata-First Processing Complete!")
...
if __name__ == "__main__":
    processor = FixedMetadataFirstProcessor()
    
    print("🚀 FIXED Metadata-First RAG Processor")
    print("=" * 50)
    print("Fixed: Correct path parsing for season folder discovery")
    print("=" * 50)
    
    confirm = input("Start fixed metadata-first processing? (y/n): ").lower()
    if confirm == 'y':
